...
export default function TasksWidget({ title = 'Мои задачи', trackerConnected = false }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/tracker/tasks')
      .then(r => r.json())
      .then(data => { if (data.error) throw new Error(data.error); setTasks(data.tasks || []); })
  ...
  const hiddenTasks = expanded ? [] : tasks.slice(VISIBLE_COUNT);
  ...
  return (
    <div className="bg-craft-surface border border-craft-border rounded-xl overflow-hidden transition-colors duration-200 hover:border-craft-border2">
      ...
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-craft-border">
        ...
        {tasks.length > 0 && <span className="text-2xs text-white/25">{tasks.length}</span>}
      </div>
      ...
      {loading ? (
        ...
        <EmptyState icon="tasks" title="Трекер не подключён" sub="Добавьте TRACKER_ORG_ID в переменные окружения" />
      ) : tasks.length === 0 ? (
        <div className="px-5 py-10 text-center">
                ...
                <div>
                  <div className="divide-y divide-white/[0.03]">
                    {tasks.slice(VISIBLE_COUNT).map((task, i) => (
                      <TaskRow key={task.key || (VISIBLE_COUNT + i)} task={task} index={VISIBLE_COUNT + i} />
                    ))}
                  </div>
                </div>
              </div>
...
function TaskRow({ task, index }) {
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-2.5 hover:bg-white/[0.02] transition-all duration-200 group">
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[13px] text-white/55 truncate group-hover:text-white/80 transition-colors duration-200">{task.summary}</div>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        {task.deadline && (
          <span className={`text-2xs transition-colors duration-200 ${isOverdue ? 'text-craft-red' : 'text-white/20'}`}>{fmtDate(task.deadline)}</span>
        )}
        <span className={`text-2xs px-2 py-0.5 rounded transition-all duration-200 ${status.bg} ${status.text}`}>{status.label}</span>
      </div>
    </div>
  );
}
    ...
    <div className="px-5 py-10 text-center">
      <div className="w-10 h-10 mx-auto mb-3 rounded-xl bg-white/[0.03] flex items-center justify-center">
        <svg className="w-5 h-5 text-white/10" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.2">
          <rect x="3" y="4" width="14" height="12" rx="2" /><path d="M7 9h6M7 12h4" />
        </svg>
      </div>
