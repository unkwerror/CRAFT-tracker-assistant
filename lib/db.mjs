...
export async function getWidgetPreferences(userId) {
  const { rows } = await query(
    'SELECT widget_key, settings FROM widget_preferences WHERE user_id = $1',
    [userId]
  );
  // Return as { widgetKey: settings } map for easy consumption
  return Object.fromEntries(rows.map(r => [r.widget_key, r.settings]));
}

export async function upsertWidgetPreference(userId, widgetKey, settings) {
  const { rows } = await query(
    `INSERT INTO widget_preferences (user_id, widget_key, settings)
     VALUES ($1, $2, $3)
     ON CONFLICT (user_id, widget_key) DO UPDATE
       SET settings = $3, updated_at = NOW()
     RETURNING *`,
    [userId, widgetKey, JSON.stringify(settings)]
  );
  return rows[0];
}
