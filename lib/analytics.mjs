...
export function calcVelocityFromChangelog(changelogs) {
...
function encodeLeadFeatures(lead) {
  const cf = lead.customFields || {};
  const budget = Number(findField(cf, 'budgetKP', 'Бюджет КП', 'contractSum', 'Сумма договора') || 0);
  const source = findField(cf, 'leadSource', 'Источник лида') || '';
  const objType = findField(cf, 'objectType', 'Тип объекта') || '';
  const sk = lead.statusKey || 'newLead';
  ...
  return [
    ...
    STAGE_ORDER.indexOf(sk) >= 0 ? STAGE_ORDER.indexOf(sk) : 0,
  ];
}

export function trainScoringModel(historicalLeads) {
  const wonLeads = historicalLeads.filter(l => WON_STAGES.includes(l.statusKey));
  const lostLeads = historicalLeads.filter(l => LOST_STAGES.includes(l.statusKey));
  const total = wonLeads.length + lostLeads.length;
  if (total < 20) return null;

  const X = [];
  const Y = [];
  for (const l of wonLeads) { X.push(encodeLeadFeatures(l)); Y.push(1); }
  for (const l of lostLeads) { X.push(encodeLeadFeatures(l)); Y.push(0); }
...
export function predictWithModel(model, lead) {
  if (!model) return null;
  try {
    const features = encodeLeadFeatures(lead);
...
export function trainConversionModel(historicalLeads) {
...
export function predictConversion(model, lead) {
  if (!model) return null;
  try {
    const features = encodeLeadFeatures(lead);
...
export function detectAnomalies(leads) {
  ...
  const activeLeads = leads.filter(l =>
    l.createdAt && !['projectOpened', 'rejected', 'closed'].includes(l.statusKey)
  );
...
export function segmentLeads(leads, k = 4) {
  ...
  const features = validLeads.map(l => {
    ...
    const stageIdx = STAGE_ORDER.indexOf(l.statusKey || 'newLead');
    return [
      budget > 0 ? Math.log10(budget) : 0,
      Math.min(ageDays, 365) / 365,
      stageIdx >= 0 ? stageIdx / (STAGE_ORDER.length - 1) : 0,
    ];
  });
  ...
  try {
    ...
    for (let i = 0; i < result.centroids.length; i++) {
      ...
      const budgets = members.map(l => {
        const cf = l.customFields || {};
        return Number(findField(cf, 'budgetKP', 'Бюджет КП', 'contractSum', 'Сумма договора') || 0);
      });
...
export function cohortAnalysis(leads, groupBy = 'month') {
  const cohorts = {};
  for (const l of leads) {
    if (!l.createdAt) continue;
  ...
  return Object.entries(cohorts)
    ...
    .map(([period, members]) => {
      const won = members.filter(l => WON_STAGES.includes(l.statusKey));
      const lost = members.filter(l => LOST_STAGES.includes(l.statusKey));
      const budgets = members.map(l => {
        const cf = l.customFields || {};
        return Number(findField(cf, 'budgetKP', 'Бюджет КП', 'contractSum', 'Сумма договора') || 0);
      }).filter(b => b > 0);
...
export function managerPerformance(leads) {
  const byManager = {};
  for (const l of leads) {
  ...
  return Object.entries(byManager).map(([manager, mLeads]) => {
    const won = mLeads.filter(l => WON_STAGES.includes(l.statusKey));
    const lost = mLeads.filter(l => LOST_STAGES.includes(l.statusKey));
...
export function winLossAnalysis(leads) {
  const won = leads.filter(l => WON_STAGES.includes(l.statusKey));
  const lost = leads.filter(l => LOST_STAGES.includes(l.statusKey));
  ...
  for (const l of [...won, ...lost]) {
    const cf = l.customFields || {};
    const source = findField(cf, 'leadSource', 'Источник лида') || 'Неизвестно';
    const objType = findField(cf, 'objectType', 'Тип объекта') || 'Неизвестно';
    const isWon = WON_STAGES.includes(l.statusKey);
...
export function conversionByPeriod(leads, period = 'month') {
  const buckets = {};
  for (const l of leads) {
    if (!l.createdAt) continue;
    if (!WON_STAGES.includes(l.statusKey) && !LOST_STAGES.includes(l.statusKey)) continue;
    const d = new Date(l.createdAt);
    const key = period === 'quarter'
      ? `${d.getFullYear()}-Q${Math.floor(d.getMonth() / 3) + 1}`
      : `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
    if (!buckets[key]) buckets[key] = { won: 0, lost: 0 };
    if (WON_STAGES.includes(l.statusKey)) buckets[key].won++;
...
export function buildAnalyticsBundle(leads) {
  ...
  const creationTrend = calcCreationTrend(leads);

  let revenueForecast = null;
  const monthlyBuckets = buildMonthlyRevenueSeries(leads);
  ...
  return {
    ...
    creationTrend,
    cohorts: cohortAnalysis(leads),
    managerKPI: managerPerformance(leads),
    winLoss: winLossAnalysis(leads),
    segments: segmentLeads(leads),
    conversionByPeriod: conversionByPeriod(leads),
    revenueForecast,
  };
}

function buildMonthlyRevenueSeries(leads, months = 6) {
  const now = new Date();
...
export { STAGE_ORDER, STAGE_LABELS, PIPELINE_WEIGHTS };
