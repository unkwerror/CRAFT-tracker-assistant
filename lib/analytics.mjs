...
export function scoreLead(lead) {
  const budget = lead.customFields?.budgetKP;
  const source = lead.customFields?.leadSource;
  const objType = lead.customFields?.objectType;
...
export function forecastRevenue(leads) {
  ...
  for (const l of leads) {
    const sk = l.statusKey || 'unknown';
    const budget = Number(l.customFields?.budgetKP || l.customFields?.contractSum || 0);
