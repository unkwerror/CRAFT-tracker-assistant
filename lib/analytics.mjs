 ...
 * ml-logistic-regression — conversion prediction (won/lost) [retained, not called from API since v2.8.0]
...
// ─── In-memory ML model cache (TTL: 10 minutes) ───
// Strategy: in-memory Map per module (persists across requests within one Node.js process).
// Preferred over DB JSONB: Random Forest serializes to 100–500KB per model,
// adding more DB latency than training saves. For Render Starter (1 instance, 1–2 deploys/day)
// in-memory is optimal. Models retrain only at cold start and after 10-min expiry.
const _modelCache = new Map();
const _MODEL_TTL_MS = 10 * 60 * 1000;

function _getCachedModel(key) {
  const entry = _modelCache.get(key);
  if (!entry) return null;
  if (Date.now() - entry.ts > _MODEL_TTL_MS) {
...
function _setCachedModel(key, model) {
  _modelCache.set(key, { model, ts: Date.now() });
}
...
// ─── 2c. Velocity benchmark — period-over-period comparison ───

/**
 * Approximate industry velocity for architecture/design bureaus (days per stage).
 * Based on typical project acquisition cycles in Russia.
 * Treat as orientation only — marked isIndustryEstimate: true on each entry.
 */
export const INDUSTRY_VELOCITY_BENCHMARK = {
  newLead:       { medianDays: 3,  label: 'Новый лид',       isIndustryEstimate: true },
  qualification: { medianDays: 14, label: 'Квалификация',    isIndustryEstimate: true },
  proposal:      { medianDays: 21, label: 'КП отправлено',   isIndustryEstimate: true },
  negotiation:   { medianDays: 30, label: 'Переговоры',      isIndustryEstimate: true },
  contract:      { medianDays: 10, label: 'Договор',         isIndustryEstimate: true },
  projectOpened: { medianDays: 7,  label: 'Проект открыт',   isIndustryEstimate: true },
};

/**
 * Compare velocity between two chronological halves of the lead list.
 *
 * Split strategy: 50/50 by creation date (not calendar months).
 * Rationale: calendar months produce unequal sample sizes for a ~40–80 lead CRM
 * (e.g. 5 leads last month vs 30 this month → biased medians).
 * Median-split guarantees both halves have the same number of leads → stable stats.
 *
 * Returns per-stage: recentMedian, previousMedian, trend, deltaPercent,
 * plus optional industryMedian from INDUSTRY_VELOCITY_BENCHMARK.
 */
export function calcVelocityBenchmark(leads, { stageConfig } = {}) {
  const sc = stageConfig || DEFAULT_CRM_STAGES;
  const sorted = leads
  ...
  const olderLeads = sorted.slice(0, mid);
  const recentLeads = sorted.slice(mid);

  const prevVelocity = calcVelocity(olderLeads, { stageConfig: sc });
  const recentVelocity = calcVelocity(recentLeads, { stageConfig: sc });

  const prevByStage = Object.fromEntries(prevVelocity.map(v => [v.stage, v]));
  const STABLE_THRESHOLD_PCT = 10;

  return recentVelocity.map(r => {
    const prev = prevByStage[r.stage];
    const industry = INDUSTRY_VELOCITY_BENCHMARK[r.stage];

    const previousMedian = prev?.median ?? null;
    const deltaPercent = previousMedian != null && previousMedian > 0
      ? Math.round(((r.median - previousMedian) / previousMedian) * 100)
      : 0;

    let trend = 'stable';
    if (previousMedian != null) {
      if (deltaPercent > STABLE_THRESHOLD_PCT) trend = 'slower';
      else if (deltaPercent < -STABLE_THRESHOLD_PCT) trend = 'faster';
    }

    return {
      stage: r.stage,
      label: r.label,
      recentMedian: r.median,
      recentCount: r.count,
      previousMedian,
      previousCount: prev?.count ?? null,
      trend,
      deltaPercent,
      ...(industry ? { industryMedian: industry.medianDays, isIndustryEstimate: true } : {}),
    };
  });
}
...
/**
 * Cached version of trainScoringModel.
 * Returns a cached model if trained within the last 10 minutes,
 * otherwise trains a new one and caches it.
 * @param {Array} leads
 * @param {Object} config — { fieldMap, stageConfig }
 * @param {string} [cacheKey] — override for multi-queue setups
 */
export function getOrTrainScoringModel(leads, config = {}, cacheKey = 'scoring:crm') {
  const cached = _getCachedModel(cacheKey);
  if (cached) {
    console.log(`[analytics] Scoring model served from cache (key=${cacheKey})`);
    return cached;
  }
  const model = trainScoringModel(leads, config);
  if (model) {
    _setCachedModel(cacheKey, model);
    console.log(`[analytics] Scoring model trained and cached (key=${cacheKey})`);
  }
  return model;
}

// ─── 5. Conversion prediction (Logistic Regression) ───
// NOTE: Not called from API since v2.8.0. Retained for future use.
// Removed from route because LogReg at small scale (< 30 examples) is numerically
// unstable and produced unreliable predictions. See ROADMAP v3 Phase 5.3.
...
/**
 * Auto-labels a K-means segment based on its centroid characteristics.
 * Labels are tuned for an architecture/design bureau sales context:
 *
 * - "Горячий VIP"      — high budget + fresh lead → call today, top priority
 * - "Крупный на паузе" — high budget + stalled → reanimate, don't let it go cold
 * - "Быстрый старт"    — small budget + fresh → expect quick decision, close fast
 * - "Зависший малый"   — small budget + old → decide to close or drop
 * - "Средний проект"   — everything else, standard pipeline work
 *
 * Budget thresholds (architecture fees in RUB):
 *   ≥ 5M = large (residential complex, large commercial)
 *   < 500K = small (interior, minor elements)
 * Age thresholds: ≤ 60 days = fresh, > 90 days = old
 */
function _autoLabelSegment(seg) {
  const budget = seg.avgBudget || 0;
  const age = seg.medianAge || 0;

  const isLarge = budget >= 5_000_000;
  const isSmall = budget < 500_000;
  const isFresh = age <= 60;
  const isOld = age > 90;

  if (isLarge && isFresh) return 'Горячий VIP';
  if (isLarge && isOld) return 'Крупный на паузе';
  if (isLarge) return 'Крупный проект';
  if (isSmall && isFresh) return 'Быстрый старт';
  if (isSmall && isOld) return 'Зависший малый';
  return 'Средний проект';
}
...
export function segmentLeads(leads, k = 4, { fieldMap, stageConfig } = {}) {
  ...
  try {
    ...
    for (let i = 0; i < result.centroids.length; i++) {
      ...
      seg.label = _autoLabelSegment(seg);
      segments.push(seg);
    }
...
export function buildAnalyticsBundle(leads, { fieldMap, stageConfig } = {}) {
  ...
  return {
    ...
    velocityBenchmark: calcVelocityBenchmark(leads, cfg),
    scores,
